...
**Krewire Libraries** is a monorepo of modular, reusable Go libraries for the Krewire ecosystem — `github.com/krewire/libs`. It provides the shared building blocks behind [`framework`](https://github.com/krewire/framework), [`mdbind`](https://github.com/krewire/mdbind), and [`krewire`](https://github.com/krewire/krewire).
...
Together `core` (declarative) + `kern` (imperative) form the **central control** of the ecosystem: every repo imports `core` for types/rules, and `framework`/`krewire` compose via `kern`. `config` + `validate` enforce the single-config rule — every workload from `cli` to `infra` is described in one `krewire.yaml` and validated before `krewire build` / `krewire deploy`.
...
- [framework](https://github.com/krewire/framework) — unified framework (`cli`/`web`/`ui`/`runtime`/`worker`/`service`/`infra`)
- [mdbind](https://github.com/krewire/mdbind) — book/site builder on `framework/web`
- [krewire](https://github.com/krewire/krewire) — devtool CLI for all kinds
